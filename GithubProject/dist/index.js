...
let resultCards = resultContainer.querySelectorAll(".result-card");
const errorMessage = document.querySelector(".error-message");
//? Helper Function | Reusable Functions
async function useFetchHook(url, options) {
    const res = await fetch(url, options);
    if (!res.ok)
        throw new Error(`Error in fetch users data - ${res.status}`);
...
//? Init Function
(async function () {
    useFetchHook("https://api.github.com/users", {}).then((usersInfo) => {
        let html = '<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 sm:gap-10 mx-5 sm:mx-0 mt-10">';
